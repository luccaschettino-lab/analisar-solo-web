...
export default function PainelLateral({
  ...
  if (recolhido) {
    return (
      <button
        ...
        className="absolute left-3 top-3 z-[1100] min-h-11 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow hover:bg-slate-50"
      >
        ☰ Painel
      </button>
    )
  ...
  return (
    <>
      {/* No celular o painel cobre o mapa. Este véu escurece o que sobra e
          fecha ao toque — sem ele, o usuário fica sem saída óbvia. */}
      <div
        onClick={aoAlternar}
        aria-hidden="true"
        className="absolute inset-0 z-[1090] bg-slate-900/20 md:hidden"
      />

      {/* Celular: gaveta ocupando quase toda a largura, com o mapa espiando na
          borda. Tablet e desktop: a coluna fixa de sempre.
          z acima dos controles do Leaflet (1000) e abaixo dos modais (2000). */}
      <aside className="absolute left-0 top-0 z-[1100] flex h-full w-[85%] max-w-sm flex-col border-r border-slate-200 bg-white/95 shadow-lg backdrop-blur md:w-80">
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-2 md:py-3">
        <h2 className="text-sm font-semibold text-slate-900">Propriedade</h2>
        <button
          onClick={aoAlternar}
          aria-label="Recolher painel"
          title="Recolher painel"
          className="-mr-2 min-h-11 min-w-11 rounded text-lg text-slate-500 hover:bg-slate-100 md:min-h-0 md:min-w-0 md:px-2 md:py-1 md:text-sm"
        >
          ⟨
        </button>
      </div>
